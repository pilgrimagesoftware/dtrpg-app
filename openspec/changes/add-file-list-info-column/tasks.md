...
- [ ] 1.1 Add a PDF page-count crate (e.g. `lopdf`) to `crates/dtrpg-ui/Cargo.toml`, pinned to
      its current latest stable version.
- [ ] 1.2 Add the `image` crate (or confirm it's already a transitive dependency at a usable
      version) for `image::image_dimensions()`.

## 2. Metadata extraction

- [ ] 2.1 Add a `FileInfo` enum/type (e.g. `FileInfo::PageCount(u32)`,
      ...
      `"1920x1080"`).
- [ ] 2.2 Add `extract_pdf_page_count(path: &Path) -> Option<u32>`, `extract_zip_file_count(path:
...
- [ ] 2.3 Add a format-dispatch function `compute_file_info(path: &Path, format: &str) ->
...
- [ ] 3.1 Add a `file_info_cache: HashMap<(Arc<str>, usize), FileInfo>` field to
      `LibraryController`, keyed by `(entry_id, row_ix)`.
- [ ] 3.2 Add `LibraryController::file_info(&mut self, entry_id: &Arc<str>, row_ix: usize) ->
...
- [ ] 3.3 Invalidate the cache entry for `(entry_id, row_ix)` wherever a file's `downloaded` flag
      transitions (re-download start/completion), alongside existing per-file state resets.

## 4. UI: Info column

- [ ] 4.1 Add an `"info"` column to `item_list_columns()` in `detail_panel_view.rs`, positioned
      after `"status"`, using a new i18n key for its header label.
- [ ] 4.2 Extend `ItemListDelegate::render_td`'s `match col_ix` with a new arm (col_ix 4) that
...
- [ ] 4.3 Update `ItemListDelegate::columns_count` to reflect the new column count.

## 5. i18n

- [ ] 5.1 Add `detail.item_list_column_info` header key to `en.yaml`.
- [ ] 5.2 Add matching translations to `fr.yaml` and `de.yaml`.

## 6. Verification

- [ ] 6.1 Unit tests for `extract_pdf_page_count`, `extract_zip_file_count`, and
...
- [ ] 6.2 Unit test for `LibraryController::file_info` caching: verify a second call for the same
      ...
      still returns).
- [ ] 6.3 `cargo test --workspace`, `cargo clippy --all-targets --all-features -- -D warnings`,
